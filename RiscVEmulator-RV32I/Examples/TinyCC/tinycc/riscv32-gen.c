...
ST_FUNC void load(int r, SValue *sv)
{
    ...
    if (fr & VT_LVAL) {
        ...
        if (fc || (rr != rb) || do32bit || (fr & VT_SYM))
          EI(0x13 | do32bit, 0, rr, rb, SIGN11(fc)); // addi R, x0|R, FC
    } else if (v == VT_LOCAL) {
...
ST_FUNC void gfunc_prolog(Sym *func_sym)
{
    ...
    while ((sym = sym->next) != NULL) {
        ...
        if (areg[prc[1] - 1] >= 8
            ...
            loc -= regcount * 4; // XXX could reserve only 'size' bytes
            param_addr = loc;
            for (i = 0; i < regcount; i++) {
                if (areg[prc[1+i] - 1] >= 8) {
                    assert(i == 1 && regcount == 2 && !(addr & 3));
                    ...
                    ES(0x23, 2, 8, 10 + areg[0]++, loc + i*4); // sw aX, loc(s0) // XXX
                }
            }
        }
    ...
    if (func_var) {
        for (; areg[0] < 8; areg[0]++) {
            num_va_regs++;
            ES(0x23, 2, 8, 10 + areg[0], -4 + num_va_regs * 4); // sw aX, loc(s0)
        }
    }
#ifdef CONFIG_TCC_BCHECK
    if (tcc_state->do_bounds_check)
...
ST_FUNC void gfunc_epilog(void)
{
    ...
    loc = (loc - num_va_regs * 4);
    d = v = (-loc + 15) & -16;

    EI(0x13, 0, 2, 8, num_va_regs * 4); // addi sp, s0, num_va_regs*4
    ...
    if (v >= (1 << 11)) {
        d = 16;
        EI(0x13, 0, 8, 2, d - num_va_regs * 4);      // addi s0, sp, d
    ...
    ES(0x23, 2, 2, 1, d - 4 - num_va_regs * 4);  // sw ra, d-4(sp)
    ES(0x23, 2, 2, 8, d - 8 - num_va_regs * 4); // sw s0, d-8(sp)
    if (v < (1 << 11))
      EI(0x13, 0, 8, 2, d - num_va_regs * 4);      // addi s0, sp, d
    else
...
/* gen_opl is provided by tccgen.c for PTR_SIZE==4 (64-bit ops via library calls) */
