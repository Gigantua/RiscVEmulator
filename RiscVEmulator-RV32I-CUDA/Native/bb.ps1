...
cmd /c "call `"$vc`" >nul 2>&1 && nvcc -O3 -std=c++20 --expt-relaxed-constexpr -arch=sm_86 $defs -diag-suppress 549 -shared -cudart static -o `"$out`" `"$src`" -lcuda 2>&1" | Select-String -Pattern "error" -Context 0,2
