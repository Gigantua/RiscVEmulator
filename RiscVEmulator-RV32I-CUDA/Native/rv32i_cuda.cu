...
enum { RC_ALUI=0x13, RC_ALUR=0x33, RC_MULC=0x0B, RC_LOAD=0x03, RC_BR=0x63, RC_ST=0x23,
       ...
       RC_PALEXP=0x4F, RC_TEXSPAN=0x59, RC_ILL=0x00 };
       // RC_TEXSPAN: texture-mapped span loop (R_DrawSpan idiom): pix=cmap[tex[((y>>shY)&mask)+((x<<shX1)>>shX2)]];
       //   dst++=pix; x+=xstep; y+=ystep → one native whole-loop uop. Params (regs/offsets/shifts read from the
       //   matched instrs, NOT hardcoded) live in the g_ext side array. Generic — keys on the idiom structure.
...
static void rvcud_build(RvcudBuild& B) {
    ...
    for (size_t i = 0; i < B.w0.size(); i++) {
        ...
        else if (cls == RC_TEXSPAN) {                                // w1 = full fall-through uop index (params live in ext[])
            uint32_t tw = B.tgt[i];
            B.w1[i] = ((int)tw < N && B.pc2uop[tw] != RC_BADUOP) ? B.pc2uop[tw] : 0xFFFFFFFFu;
        }
    }
