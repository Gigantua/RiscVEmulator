...
string src = Path.Combine(root, "Examples_CUDA", "CudaPeriphTest", "Programs", "periph_test.c");
...
// M extension (mul/div/rem) + A extension (amoadd.w) results, via UART.
uint mul = unchecked(0x12345678u * 0x9ABCDEF0u);
Check("M: MUL",            o.Contains($"MUL=0x{mul:x8}"));
Check("M: DIVU",           o.Contains($"DIVU=0x{(1000000u / 7u):x8}"));
Check("M: REMU",           o.Contains($"REMU=0x{(1000000u % 7u):x8}"));
Check("M: DIV (signed)",   o.Contains($"DIVS=0x{unchecked((uint)(-1000000 / 7)):x8}"));
Check("A: amoadd.w",       o.Contains("AMOOLD=0x00000064") && o.Contains("AMONEW=0x00000069"));
