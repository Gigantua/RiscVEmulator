...
static void putc_(int c) { MMIO8(UART) = (uint8_t)c; }
...
void _start(void) {
    ...
    // ── M extension (mul/div/rem) + A extension (amoadd.w) self-check ──
    uint32_t mul = 0x12345678u * 0x9ABCDEF0u;   // MUL
    uint32_t du  = 1000000u / 7u;               // DIVU
    uint32_t ru  = 1000000u % 7u;               // REMU
    int      ds  = -1000000 / 7;                // DIV (signed)
    puts_("MUL=");  puthex(mul);
    puts_(" DIVU="); puthex(du);
    puts_(" REMU="); puthex(ru);
    puts_(" DIVS="); puthex((uint32_t)ds); putc_('\n');
    volatile uint32_t* amo = (volatile uint32_t*)(uintptr_t)0x00810000u;
    *amo = 100;
    uint32_t amo_old;
    __asm__ volatile ("amoadd.w %0, %2, (%1)" : "=r"(amo_old) : "r"(amo), "r"(5u) : "memory");
    puts_("AMOOLD="); puthex(amo_old); puts_(" AMONEW="); puthex(*amo); putc_('\n');
