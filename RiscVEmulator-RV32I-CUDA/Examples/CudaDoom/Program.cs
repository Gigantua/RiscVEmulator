...
int cores = 1;
bool selftest = false;
bool useRvcud = false;            // --rvcud: cross-compile the guest to the rvcud uop core
...
for (int i = 0; i < args.Length; i++)
{
    ...
    else if (args[i] == "--rvcud")    useRvcud = true;                      // run on the rvcud translated kernel
...
if (useRvcud) Console.WriteLine("rvcud: cross-compiling the guest to the CUDA uop core (--rvcud).");
...
// Skip the (multi-second) clang rebuild when doom.elf is already up to date — it's the same guest
// every run. Rebuild only if a source/header/linker-script is newer than the elf (or --rebuild given).
bool stale = args.Contains("--rebuild") || !File.Exists(elfPath);
if (!stale)
{
    DateTime elfTime = File.GetLastWriteTimeUtc(elfPath);
    foreach (string f in Directory.GetFiles(runtimeDir, "*.c").Concat(Directory.GetFiles(runtimeDir, "*.h"))
             .Concat(Directory.GetFiles(programsDir, "*.c")).Concat(Directory.GetFiles(programsDir, "*.h"))
             .Append(linkerLd))
        if (File.Exists(f) && File.GetLastWriteTimeUtc(f) > elfTime) { stale = true; break; }
}
if (!stale)
    Console.WriteLine($"  doom.elf: cached ({new FileInfo(elfPath).Length:N0} bytes; pass --rebuild to force)");
else
{
...
if (args.Contains("--bench"))
{
    ...
    emu.UseRvcud = useRvcud; emu.CommitImage();
    ...
    for (int b = 0; b < 4000 && !emu.IsHalted; b++)
    {
        ...
        if (mf >= 10) break;                         // MIPS is a rate; 10 frames is plenty (was 30)
    }
    sw.Stop();
    ...
    if (useRvcud)
    {
        ulong uops = emu.RvcudIters - itersStart;       // actual uarch uops dispatched (clock-independent count)
        double rvMips = m / sec / 1e6;                  // RV32I instructions/sec — the headline (weights are gate-exact)
        double uopMs  = uops / sec / 1e6;               // uarch core throughput (uops/sec)
        double fuse   = (double)m / uops;               // fusion ratio = RV32I instrs per uop (≥1)
        Console.WriteLine($"  => RV32I {rvMips:F2} MIPS  =  {uopMs:F2}M uops/s (uarch core)  ×  {fuse:F3} fusion" +
                          $"   |  {mf / sec:F2} FPS  (cross-check: MIPS & FPS move together on a fixed rv32i binary)");
    }
    else
...
using (var emu = new CudaEmulator(RamMB * 1024 * 1024, nCores: cores))
{
    ...
    emu.LoadBytes(WadSizeAddr, BitConverter.GetBytes((uint)wadData.Length));
    emu.LoadBytes(WadBaseAddr, wadData);
    emu.UseRvcud = useRvcud; emu.CommitImageToAllCores(StackPointer, entry);
