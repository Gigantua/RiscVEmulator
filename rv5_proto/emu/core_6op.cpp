...
public:
    ...
    uint32_t XOR(uint32_t a, uint32_t b)    { ++uops; return a ^ b; }
    uint32_t ROL(uint32_t a, uint32_t n)    { ++uops; return std::rotl(a, (int)(n & 31)); }
    ...
    // ============================================================
    uint32_t NOT(uint32_t a)             { return XOR(a, K[0]); }
    uint32_t NEG(uint32_t a)             { return ADD(NOT(a), K[1]); }
    uint32_t SUB(uint32_t a, uint32_t b) { return ADD(a, NEG(b)); }

    // Derived constants — built from primitives.
    uint32_t K32()   { return ADD(K[2], K[1]); }     // 32 = 31 + 1
    uint32_t KMSB()  { return ROL(K[1], K[2]); }     // 0x80000000 = 1 << 31
    uint32_t KFOUR() { return ADD(K[3], K[1]); }     // 4 = 3 + 1
    uint32_t KNOT1() { return NOT(K[1]); }           // 0xFFFFFFFE = ~1
...
static bool do_step(Core& c) {
    ...
    switch (opcode) {
    case 0x37: c.WX(rd, imm_u); break;                                        // LUI
    case 0x17: c.WX(rd, c.ADD(pc, imm_u)); break;                             // AUIPC
    case 0x6F:                                                                 // JAL
