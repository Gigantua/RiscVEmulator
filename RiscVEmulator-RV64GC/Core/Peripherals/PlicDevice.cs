...
namespace RiscVEmulator.Core.Peripherals
{
    ...
    public sealed class PlicDevice : IPeripheral
    {
        ...
        /// <summary>External-IRQ line sinks into the <see cref="Emulator64"/>
        /// native core — the PLIC raises/clears MEIP/SEIP through these.</summary>
        public static Action<bool> MachineIrqSink    = Emulator64.SetMachineExtIrq;
        public static Action<bool> SupervisorIrqSink = Emulator64.SetSupervisorExtIrq;

        /// <summary>Toggle MEIP/SEIP based on whether each context has a deliverable IRQ.</summary>
        private void UpdateCpuLines()
        {
            MachineIrqSink   (HighestPendingEnabled(0) != 0);
            SupervisorIrqSink(HighestPendingEnabled(1) != 0);
        }
    }
}
