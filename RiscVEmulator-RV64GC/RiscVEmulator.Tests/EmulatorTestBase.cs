...
public abstract class EmulatorTestBase
{
    ...
    protected static void CompileC(IEnumerable<string> srcFiles, string elfFile)
    {
        ...
        foreach (string t in RiscVTarget.ClangTargetArgs(RiscVTarget.ArchBase))
    ...
    /// <summary>Step the RV64 emulator in batches until it halts or the step budget runs out.</summary>
    private static void RunToHalt(Emulator64 emu, int maxSteps)
    {
        int remaining = maxSteps;
        const int batch = 250_000;
        while (remaining > 0 && !emu.IsHalted)
        {
            int executed = emu.StepN(remaining < batch ? remaining : batch);
            if (executed == 0) break;
            remaining -= executed;
        }
    }
