...
string demoC        = Path.Combine(exeDir, "Programs", "input_demo.c");

string buildDir = Path.Combine(exeDir, "build");
Directory.CreateDirectory(buildDir);

// ── Compile ──────────────────────────────────────────────────────
Console.WriteLine("Compiling input demo...");
...
string demoObj      = Path.Combine(buildDir, "input_demo.o");
string elfPath      = Path.Combine(buildDir, "input_demo.elf");
...
Console.WriteLine($"  input_demo.elf: {new FileInfo(elfPath).Length:N0} bytes");
...
void CompileObj(string src, string obj)
{
    ...
    RunClang(new[] {
        $"--target={RiscVTarget.Triple}", $"-march={RiscVTarget.ArchBase}", $"-mabi={RiscVTarget.Abi}",
