...
Console.WriteLine("TinyCC JIT Demo — RV64GC emulator");
Console.WriteLine($"Compiling TinyCC + demo for RV64GC...");
...
// Compile tinycc_demo.c with -O1 (TCC source is large; -O3 is very slow)
{
    ...
    string[] extra = {
        "-DONE_SOURCE=1", "-DTCC_TARGET_RISCV64=1",
...
// ── Link ───────────────────────────────────────────────────────────
{
    ...
    {
        $"--target={RiscVTarget.Triple}", $"-march={RiscVTarget.ArchM}", $"-mabi={RiscVTarget.Abi}",
