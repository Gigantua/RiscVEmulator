...
public sealed unsafe class LinuxSdlAudio : IDisposable
{
    ...
    // Byte-flow measurement — total PCM bytes handed to SDL.
    ...
    public void Stop()
    {
        ...
        Console.Error.WriteLine($"[audio] total PCM drained this session: {_totalBytes:N0} bytes");
    }
        ...
        if (len > 0)
        {
            ...
            if (now - _lastLogMs >= 1000)
            {
                Console.Error.WriteLine(
                    $"[audio] PCM drained: {_totalBytes:N0} bytes " +
                    $"(~{_totalBytes / (44100.0 * 2 * 2):F1}s @44.1k/2ch/16-bit)");
                _lastLogMs = now;
            }
        }
        _ctrl.Ctrl = 0;
