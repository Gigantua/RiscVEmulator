...
// ── Configuration ─────────────────────────────────────────────────
// RV64 layout: PureDOOM sizes its zone at 6*(sizeof(void*)/4) MB — 12 MB on
// a 64-bit target. The heap must therefore have >12 MB clear below the WAD,
// so the WAD sits at 24 MB inside a 32 MB RAM (was 10 MB / 16 MB for RV32).
const uint WadBaseAddr = 0x01800000;   // 24 MB
const uint WadSizeAddr = 0x017FFFFC;
const uint StackPointer = 0x017FFF00;
const int RamMB = 32;
...
// ── Parse CLI args ────────────────────────────────────────────────
var opts = new SdlWindowOptions { Title = "DOOM — RV64GC Emulator", GrabMouse = true };
...
{
    string march = enableMExt ? RiscVTarget.ArchM : RiscVTarget.ArchBase;
    var linkArgs = new List<string>
    {
        $"--target={RiscVTarget.Triple}", $"-march={march}", $"-mabi={RiscVTarget.Abi}",
...
ulong entry = ElfLoader.Load(elfData, bus);
...
bool Compile(string src, string obj, string[] extraFlags, bool mExt)
{
    ...
    string march = mExt ? RiscVTarget.ArchM : RiscVTarget.ArchBase;
