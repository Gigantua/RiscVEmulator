...
#define WAD_BASE_ADDR  0x01800000u  /* 24 MB offset into RAM (RV64 zone needs 12 MB heap) */
#define WAD_SIZE_ADDR  (*(volatile unsigned int *)0x017FFFFCu) /* size stored just before WAD */
...
void _start(void)
{
    ...
    while (1) {
        ...
        /* Advance MIDI at 140 Hz using wall-clock time */
        {
            ...
            /* Anti-spiral clamp. Every MIDI_DATA write below is a guarded-MMIO
             * fault into the host synth and costs real wall-time. A slow frame
             * makes delta_us span many ticks; draining them lengthens the next
             * frame, which queues still more ticks — a compounding loop that
             * makes each frame ~30% slower than the last until Doom crawls.
             * Capping delta_us bounds the MIDI work per frame so the feedback
             * coefficient stays < 1: the loop plateaus instead of diverging.
             * Music may skip slightly on a slow frame; the game stays live. */
            if (delta_us > 8u * MIDI_TICK_US)
                delta_us = 8u * MIDI_TICK_US;
