...
| Sv32 MMU         | full     | 2-level page-table walk + 256-entry TLB; page faults  |
| SBI (opt-in)     | full     | `rv32i_set_sbi_mode` — emulator is the M-mode firmware |
...
## MMU and SBI (Sv32 Linux)

`rv32i_core.cpp` implements the **Sv32 MMU**: a two-level page-table walk on
every fetch/load/store/AMO when `satp.MODE=1` and the effective privilege is
below M. A 256-entry direct-mapped TLB is flushed by `SFENCE.VMA` and `satp`
writes; A/D bits are updated in hardware; faults raise cause 12/13/15. When
`satp.MODE=0` — every bare-metal example, the nommu kernel — translation is a
single predicted-not-taken branch, i.e. bare physical addressing, unchanged.

`rv32i_set_sbi_mode(1)` turns the emulator into the M-mode **SBI firmware**:
the kernel boots in S-mode, S-mode `ECALL`s are serviced in-place (Base /
TIME / IPI / RFENCE / SRST / DBCN + v0.1 legacy), and all other traps route to
S-mode. This is how the MMU Linux build boots — `Examples.Linux --mmu` (add
`--sh` for a direct `rdinit=/bin/sh` root shell). The MMU kernel is an rv32ima
soft-float glibc buildroot image; `fork()` and dynamic loading work. See
`ISA_PLAYBOOK.md`.

`Examples.Linux --mmu --gui` is the graphical mode: it registers a
framebuffer, keyboard/mouse and virtio-net, opens an SDL window, and the
guest boots into **NetSurf** running fullscreen on the framebuffer (the
`rvemu-input` daemon bridges the MMIO keyboard/mouse to `/dev/input/event*`;
DHCP, ICMP and HTTP all work over the slirp NAT). The MMU rootfs, overlay
and device trees (`rvemu-mmu.dts`, `rvemu-mmu-gui.dts`) live under
`Examples/Linux/`; `LinuxBootTest` exercises the boot + `fork()` + networking.
