...
/* Heap ceiling above _heap_start. 20 MB so PureDOOM's RV64 zone — sized
 * 6*(sizeof(void*)/4) = 12 MB on a 64-bit target — fits in one malloc.
 * Examples that place a WAD/stack lower than this simply never allocate
 * that much; the limit is a guard, not a reservation. */
#define HEAP_LIMIT  (20u * 1024u * 1024u)
