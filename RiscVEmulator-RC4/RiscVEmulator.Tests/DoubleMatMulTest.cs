...
[TestClass]
public class DoubleMatMulTest : EmulatorTestBase
{
    // double_matmul.c outputs 4 labelled 3x3 matrices (same as float_matmul)
    // plus a 2x2 inverse printed as *2 (to handle half-integer results).
    private static readonly string[] ExpectedLines =
    {
        ...
        "12", "15", "18",
        "21", "24", "27",
        // 2x2 inverse of [[4,2],[3,1]]
        // inv = [[-0.5, 1.0],[1.5, -2.0]], printed as *2: [-1, 2, 3, -4]
        "INV2x2",
        "-1", "2", "3", "-4",
    };

    [TestMethod]
    public void DoubleMatrixOperationsAndInverse()
    {
        string text = RunProgramWithLibc("double_matmul");
