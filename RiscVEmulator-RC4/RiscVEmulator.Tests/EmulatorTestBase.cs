...
public abstract class EmulatorTestBase
{
    ...
    /// <summary>Compile one or more C/object files to a RISC-V ELF using
    /// the patched WSL clang (~/llvm-build/bin/clang). Runs with the
    /// `xarx` extension enabled so the RISCVReduceToFiveOpsPass actually
    /// fires.</summary>
    protected static void CompileC(IEnumerable<string> srcFiles, string elfFile)
    {
        ...
        bool ok = RiscVEmulator.Core.WslClang.Run(args, out string stderr, cwd: TestDir);
        Assert.IsTrue(ok, $"clang failed:\n{stderr}");
    }

    /// <summary>Compile a single C source to an object file via the
    /// patched WSL clang with -march=rv32i_xarx.</summary>
    protected static void CompileToObject(string srcFile, string objFile, string optLevel)
    {
        var args = new List<string>
        {
            ...
            "-Werror=int-conversion",
            $"-I{RuntimeDir}",
            $"-I{Path.Combine(RuntimeDir, "softfp")}",
            "-include",
            Path.Combine(RuntimeDir, "softfp", "rvemu_softfp_force.h"),
            "-c",
            srcFile,
            "-o",
            objFile,
        };
        bool ok = RiscVEmulator.Core.WslClang.Run(args, out string err, cwd: TestDir);
        // Keep the same error semantics as the old PSI path for the asserts below.
        string stdout = string.Empty;
        string stderr = err;
        int exitCode = ok ? 0 : 1;

        Assert.IsTrue(exitCode == 0,
            $"clang -c failed (exit {exitCode}).\nstdout: {stdout}\nstderr: {stderr}");
    }
        ...
        Action<UartDevice>? setupUart = null)
    {
        ...
        // Wait for the background drain thread to consume every queued
        // byte before the test inspects the StringBuilder. Fast programs
        // (e.g. arx_smoke) halt before the drain thread sees them; this
        // spins for at most a few ms.
        uart.FlushOutput();
