...
/* ─── RISC5 constant pool ─────────────────────────────────────────────
 * Loaded at VMA 0 by Runtime/linker.ld. `lw rd, off(x0)` with the
 * 12-bit signed offset addresses any slot. The LLVM pass replaces
 * matching ADDI immediates with LW from these slots. */
__attribute__((section(".const_pool"), used, aligned(4)))
const unsigned RVEMU_CONST_POOL[] = {
    /* 0x00 */ 0u,
    /* 0x04 */ 1u,
    /* 0x08 */ (unsigned)-1,
    /* 0x0C */ 2u,
