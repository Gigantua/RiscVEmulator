...
namespace RiscVEmulator.Core.Peripherals
{
    ...
    public class UartDevice : IPeripheral
    {
        ...
        private long _drainedBytes; // bytes already delivered to OutputHandler
        ...
        /// <summary>
        /// Wait until the drain thread has flushed all pending bytes from
        /// the TX ring to the OutputHandler / OutputStream sink. Tests
        /// call this after <c>emu.Run()</c> so that a fast-finishing
        /// program's output is fully captured before the assertions run.
        /// </summary>
        public void FlushOutput(int timeoutMs = 250)
        {
            // If no drain thread is running, drain synchronously on the
            // caller's thread. This is the common test path — see the
            // OutputHandler setter for why we avoid auto-starting the
            // background thread when only a per-char handler is wired.
            if (_drainThread == null)
            {
                var h = _outputHandler;
                while (System.Threading.Volatile.Read(ref _txTail) !=
                       System.Threading.Volatile.Read(ref _txHead))
                {
                    byte b = _txRing[(int)(_txTail & TxRingMask)];
                    _txTail++;
                    if (h != null) { try { h((char)b); } catch { } }
                }
                _drainedBytes = _txTail;
                return;
            }
            long deadline = Environment.TickCount64 + timeoutMs;
            while (System.Threading.Volatile.Read(ref _drainedBytes) !=
                   System.Threading.Volatile.Read(ref _txHead))
            {
        ...
        public Action<char>? OutputHandler
        {
            ...
            set
            {
                ...
                // Only auto-start the background drain thread if a stream
                // sink is also wired — pure per-char handlers (used in
                // tests) drain synchronously via FlushOutput/DrainNow,
                // which avoids races between two consumers of the ring.
                if (value != null && _outputStream != null) EnsureDrainThread();
            }
        }
        ...
        private void DrainLoop()
        {
            ...
            while (true)
            {
                ...
                System.Threading.Volatile.Write(ref _drainedBytes, _drainedBytes + n);
            }
        }
