...
# Project's self-built LLVM (matches Core/WslClang.cs). Override with
# RVEMU_CLANG / RVEMU_LLD env vars if you point at a different tree.
CLANG=${RVEMU_CLANG:-$HOME/llvm-rv32i/bin/clang}
LLD=${RVEMU_LLD:-$HOME/llvm-rv32i/bin/ld.lld}

# Fall back to whatever's on $PATH if the self-built tree is missing
# (helps first-run bootstrap before build-llvm.sh has finished).
[ -x "$CLANG" ] || CLANG=$(command -v clang || command -v clang-18)
[ -x "$LLD"   ] || for c in ld.lld ld.lld-18 ld.lld-20; do
