...
// Self-built LLVM prefix. If present (built once via repo-root build-llvm.sh),
// every WSL command runs with ~/llvm-rv32i/bin prepended to PATH so unsuffixed
// `clang`/`ld.lld`/`llvm-objcopy` resolve to our tree — and the kernel build
// uses LLVM_SUFFIX="" instead of "-18".
bool haveSelfBuiltLlvm = false;
...
// --toolchain=clang uses the self-built tree at ~/llvm-rv32i (see
// build-llvm.sh in repo root). If that tree is missing fall back to apt's
// clang-18 so first-run on a fresh box still works.
if (useClang)
{
    haveSelfBuiltLlvm = RunWslSilent("test -x $HOME/llvm-rv32i/bin/clang") == 0;
    if (haveSelfBuiltLlvm)
        Console.WriteLine(" --toolchain=clang : using self-built ~/llvm-rv32i");
    else
    {
        Console.WriteLine(" --toolchain=clang : self-built ~/llvm-rv32i not found, falling back to apt clang-18");
        aptPkgs.AddRange(new[] { "clang-18", "lld-18", "llvm-18" });
    }
}
...
if (useClang)
{
    ...
    // With the self-built tree we want unsuffixed tools, so LLVM_SUFFIX is
    // omitted; apt clang-18 still needs the -18 suffix to be found.
    string llvmFlags = haveSelfBuiltLlvm ? "LLVM=1" : "LLVM=1 LLVM_SUFFIX=-18";
    RunWsl($"grep -q 'LLVM=1' {wslDir}/linux/linux.mk || " +
           $"sed -i 's|^LINUX_MAKE_FLAGS = \\\\$|LINUX_MAKE_FLAGS = {llvmFlags} \\\\|' " +
...
int RunWslInner(string command, bool captureSilently, string? stdin)
{
    ...
    // If a self-built ~/llvm-rv32i tree exists, prepend it so unsuffixed
    // clang/ld.lld/llvm-* in kernel `LLVM=1` builds resolve to our tree.
    string llvmPathPrefix = haveSelfBuiltLlvm ? "$HOME/llvm-rv32i/bin:" : "";
    psi.ArgumentList.Add($"export PATH={llvmPathPrefix}/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; " + command);
