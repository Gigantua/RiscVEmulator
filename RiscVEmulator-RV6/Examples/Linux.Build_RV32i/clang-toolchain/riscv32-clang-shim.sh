...
# Pick clang + lld. Default to the self-built tree at $HOME/llvm-rv32i (see
# build-llvm.sh); override via RVEMU_LLVM_PREFIX. Falls back to PATH lookup
# of versioned names so the shim still works where only apt clang is present.
LLVM_PREFIX="${RVEMU_LLVM_PREFIX:-$HOME/llvm-rv32i}"
if [ -x "$LLVM_PREFIX/bin/clang" ] && [ -x "$LLVM_PREFIX/bin/ld.lld" ]; then
    CLANG="$LLVM_PREFIX/bin/clang"
    LLD="$LLVM_PREFIX/bin/ld.lld"
else
    for c in clang clang-18 clang-20; do
        command -v "$c" >/dev/null 2>&1 && { CLANG=$(command -v "$c"); break; }
    done
