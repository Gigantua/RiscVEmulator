...
public abstract class EmulatorTestBase
{
    ...
    /// the self-built WSL clang (see Core/WslClang.cs).</summary>
    protected static void CompileC(IEnumerable<string> srcFiles, string elfFile)
    {
        ...
        var args = new List<string>
        {
            ...
            "-nostdlib", "-nostartfiles", "-O3", "-fno-builtin",
            "-fuse-ld=lld",
            $"-I{RuntimeDir}",
            $"-Wl,-T,{LinkerScript}",
        };
    ...
    protected static void CompileToObject(string srcFile, string objFile, string optLevel)
    {
        var args = new List<string>
        {
            "--target=riscv32-unknown-elf",
            "-march=rv32i", "-mabi=ilp32",
            optLevel, "-fno-builtin",
            $"-I{RuntimeDir}",
            "-c", srcFile, "-o", objFile,
        };

        int exit = WslClang.Run(args, out string stderr);
        Assert.IsTrue(exit == 0, $"clang -c failed (exit {exit}).\nstderr: {stderr}");
    }
