...
struct Core {
    ...
    // ============================================================
    // BASE: the 6 primitives
    // ============================================================
    uint32_t ADD(uint32_t a, uint32_t b) const { return a + b; }
    uint32_t XOR(uint32_t a, uint32_t b) const { return a ^ b; }
    uint32_t ROL(uint32_t a, uint32_t n) const { return std::rotl(a, static_cast<int>(n & 31)); }
    bool     BNE(uint32_t a, uint32_t b) const { return a != b; }
    uint32_t LW (uint32_t addr) const          { return mem[addr >> 2]; }
    void     SW (uint32_t addr, uint32_t v)    { mem[addr >> 2] = v; }
